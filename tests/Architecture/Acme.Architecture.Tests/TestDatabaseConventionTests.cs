...
/// against <c>Database=acme</c> is satisfied by writing
/// <c>Database=acme_scratch</c>, which reintroduces the defect under a new
...
public class TestDatabaseConventionTests
{
    ...
    /// names a <em>server</em> and a maintenance database, never a Acme one.
    /// The database each assembly runs against is created at run time.
    /// </summary>
    private const string TheOnePlace =
        "tests/TestSupport/Acme.TestSupport/TestPostgres.cs";
    ...
    public void Only_one_test_file_carries_a_connection_string()
    {
        ...
        offenders.Should().BeEmpty(
            ...
            + "schema nothing migrated; take AcmeTestDatabase instead, and if "
