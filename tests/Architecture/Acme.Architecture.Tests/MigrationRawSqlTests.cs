...
public class MigrationRawSqlTests
{
    ...
    {
        // No hand-written SQL exists yet, so the raw-call count cannot be the
        // control here; the folder having files at all is. When the first
        // migrationBuilder.Sql arrives, tighten this to a count again.
        Repo.SourceFiles("src/Persistence/Acme.Persistence/Migrations")
            .Should().NotBeEmpty(
                "the scan runs over the migrations folder, and an empty folder "
                + "means it is reading nothing — InitialCreate should always "
                + "be there");
    }
    ...
    private static List<(string File, string Sql)> RawSqlCalls()
    {
        ...
        foreach (var path in Repo.SourceFiles("src/Persistence/Acme.Persistence/Migrations")
