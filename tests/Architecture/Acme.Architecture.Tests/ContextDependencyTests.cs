...
public class ContextDependencyTests
{
    ...
    private static readonly string[] Kernel =
        ["Acme.SharedKernel", "Acme.Platform.Contracts", "Acme.Storage"];

    /// <summary>
    /// <b>The 0 edges.</b> Each key is a context whose <c>*.Domain</c> project
    /// may reference the <c>*.Domain</c> of every context listed, and no other.
    /// </summary>
    /// <remarks>
    /// Two contexts, no edges between them: <c>Platform</c> is tenancy and
    /// identity, and it reaches the product domain only through
    /// <c>Platform.Contracts</c> (ADR-041); <c>DocumentManagement</c> stands on
    /// the kernel alone. The first cross-context edge is a decision, argued
    /// for here — the RegOS codebase this derives from grew to 32 edges one
    /// deliberate entry at a time.
    /// </remarks>
    private static readonly Dictionary<string, string[]> DomainMayReference = new()
    {
        // Platform is tenancy and identity; it reaches the product domain
    ...
    /// A number in prose drifts silently, which is the one failure mode this
    /// whole test class exists to prevent.
    /// </summary>
    /// <remarks>
    ...
    {
        DomainMayReference.Sum(x => x.Value.Length).Should().Be(0,
            because: "the summary on DomainMayReference says 0 edges, and a "
    ...
    /// it is a query over <c>AcmeDbContext</c> which
    ...
    public void An_application_references_only_its_own_domain_and_persistence()
    {
        Offenders("Application", (context, reference) =>
                reference == "Acme.Persistence"
                || reference == $"Acme.{context}.Domain")
            .Should().BeEmpty(
    ...
    public void An_infrastructure_references_only_its_own_slice_and_persistence()
    {
        Offenders("Infrastructure", (context, reference) =>
                reference == "Acme.Persistence"
                || reference == $"Acme.{context}.Domain"
                || reference == $"Acme.{context}.Application")
            .Should().BeEmpty(
    ...
    /// the special case.
    /// <para>
    /// <b>Cross-context <em>reads</em> compose over <c>AcmeDbContext</c>; cross-context
    ...
    /// <c>AcmeDbContext</c>. What it catches is the shape that actually arrives
    ...
    public void A_context_never_names_another_contexts_repository()
    {
        ...
        offenders.Should().BeEmpty(
            ...
            + "read over AcmeDbContext instead — and if you need a WRITE in "
    ...
    private static readonly Regex RepositoryInterface = new(
        @"Acme\.(\w+)\.Domain[\w.]*\.(I\w+Repository)", RegexOptions.Compiled);

    /// <summary><c>src/Process/Acme.Process.Application/…</c> → <c>Process</c>.</summary>
    ...
    public void The_projects_are_actually_being_read()
    {
        ...
        projects.Should().HaveCountGreaterThan(9,
            "Acme has two bounded contexts of up to four projects each, "
    ...
    /// <summary>
    /// Every project under <c>src/</c> and the Acme projects it references, by
    ...
    /// <summary><c>Acme.Registration.Infrastructure</c> → <c>Registration</c>.</summary>
    ...
    /// <summary><c>Acme.Registration.Infrastructure</c> → <c>Infrastructure</c>.</summary>
