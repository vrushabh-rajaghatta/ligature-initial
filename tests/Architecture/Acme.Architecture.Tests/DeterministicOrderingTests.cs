...
/// purpose — if Acme later prefers natural keys, or some other uniqueness
...
/// </para>
/// <para>
/// <b>Scoped to read paths</b> — classes that hold a <c>AcmeDbContext</c>. That
...
public class DeterministicOrderingTests
{
    ...
    {
        Orderings().Should().HaveCountGreaterThan(3,
            "Acme had a handful of orderings on read paths when this was "
            + "written; zero means the scan stopped matching rather than that "
    ...
    private static List<Ordering> Orderings()
    {
        ...
        foreach (var path in Repo.SourceFiles("src"))
        {
            ...
            if (!text.Contains("AcmeDbContext", StringComparison.Ordinal))
                continue;
