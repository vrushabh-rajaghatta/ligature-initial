...
public sealed class IdentityConventionTests
{
    ...
    /// </summary>
    // Empty from day one, and it stays that way: this codebase started after
    // the convention, so no identity predates it. Adding an entry here to
    ...
    /// </summary>
    // Empty today: no flat master data (ADR-043 §2 shape — deterministic ids,
    // no children, no lifecycle) exists in this codebase yet. The first
    // catalogue admitted here arrives with its own ADR.
    ...
    public void Every_entity_identity_derives_from_StronglyTypedId()
    {
        ...
        offenders.Should().BeEmpty(
            ...
            + "src/Interaction/Acme.Interaction.Domain/Commitments/CommitmentId.cs. "
