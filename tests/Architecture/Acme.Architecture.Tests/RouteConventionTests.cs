...
/// health checks, OpenAPI and static files. Acme started with it, and the
...
public sealed class RouteConventionTests
{
    ...
    /// </summary>
    // Empty from day one: every route in this codebase was written under the
    // rule. Adding an entry here to unblock new code defeats the mechanism.
    ...
    public void Every_route_is_under_the_api_prefix()
    {
        ...
        foreach (var file in Repo.SourceFiles("src/Host/Acme.Api/Endpoints"))
        {
        ...
        offenders.Should().BeEmpty(
            "every Acme route lives under /api (slice-conventions.md SC-001). "
    ...
    {
        var live = Repo.SourceFiles("src/Host/Acme.Api/Endpoints")
            .Select(Repo.CodeOf)
