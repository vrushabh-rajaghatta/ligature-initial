...
/// Acme.Persistence"</em> — and <b>no test project references it directly</b>.
...
{
    private const string Persistence = "Acme.Persistence";
    private const string TestSupport = "Acme.TestSupport";

    /// <summary>
    /// <b>Reach, not reference.</b> A test project sees <c>AcmeDbContext</c>
    ...
    public void A_test_project_that_can_reach_persistence_takes_a_test_database()
    {
        ...
        offenders.Should().BeEmpty(
            "a suite that can open a AcmeDbContext can run against a schema "
            + $"nobody migrated; reference {TestSupport} and take a "
            + "AcmeTestDatabase (ADR-064)");
    }
    ...
    public void The_test_projects_are_actually_being_read()
    {
        ...
        projects.Should().HaveCountGreaterThan(5,
            "Acme had 7 test projects when this was written");

        projects.Count(project => Reaches(project, Persistence))
            .Should().BeGreaterThan(2,
                "four of them touched the database when this was written; a "
