...
public sealed class TenantProvisioningTests : IAsyncLifetime
{
    ...
    private readonly AcmeApiFactory _factory;
    private readonly HttpClient _client;

    private Session _platform = default!;
    private readonly List<Guid> _tenantIds = [];

    public TenantProvisioningTests(AcmeApiFactory factory)
    {
        _factory = factory;
    ...
    public async Task InitializeAsync()
    {
        (_platform, var response) = await Session.LoginAsync(
            _client, "platform@acme.local", "platform-password");
    ...
    public async Task DisposeAsync()
    {
        ...
        foreach (var id in _tenantIds)
        {
            ...
            await _factory.Users.ExecuteAsync(
                """DELETE FROM "Documents" WHERE "TenantId" = @id""", id);
            await _factory.Users.ExecuteAsync(
    ...
    public async Task Provisioning_creates_a_tenant_and_its_invited_admin()
    {
        ...
        // Their workspace starts empty (ADR-060). Provisioning asserts no
        // data on the customer's behalf; the administrator records the first
        // document, and nothing arrives here carrying the tenant's guid.
        var documents = await admin.SendAsync(
            _client, HttpMethod.Get, "/api/documents");
        var workspace = await documents.Content
            .ReadFromJsonAsync<List<Dictionary<string, object>>>();

        workspace.Should().BeEmpty();
    }
