...
public sealed class TenantIsolationTests : IAsyncLifetime
{
    ...
    private AcmeDbContext ContextFor(TenantId tenant) =>
        new(Options(), new FakeTenantContext(tenant));

    private AcmeDbContext ContextWithoutIdentity() =>
        new(Options());
    ...
    [Fact]
    public async Task A_document_belongs_to_one_tenant()
    {
        // Even the *names* of another tenant's documents are sensitive.
        // Created bare, queried bare — the filter does all the isolating.
        var document = Document.Create(
            _tenantA, $"Isolation Test {Guid.NewGuid():N}");

        await using (var contextA = ContextFor(_tenantA))
        {
            contextA.Documents.Add(document);
        ...
        try
        {
            ...
            (await contextA.Documents.AnyAsync(x => x.Id == document.Id))
                .Should().BeTrue();
            (await contextB.Documents.AnyAsync(x => x.Id == document.Id))
                .Should().BeFalse(
                    "tenant B must not even learn the document exists");
        }
        finally
        {
