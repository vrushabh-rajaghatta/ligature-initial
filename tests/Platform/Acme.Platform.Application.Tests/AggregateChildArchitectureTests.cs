...
public sealed class AggregateChildArchitectureTests
{
    ...
    // — which read as a test that talks to a database and does not, and which
    // TestDatabaseConventionTests correctly refused to distinguish from one
    // that does.
    private static AcmeDbContext BareContext() =>
        new(new DbContextOptionsBuilder<AcmeDbContext>()
            .UseNpgsql()
            .Options);
    ...
    /// </para>
    /// </remarks>
    // Empty from day one: every child in this codebase was written with a
    // required FK to its parent. Shrink-only in both directions — it may not
    ...
    private static List<string> SeverableChildren(AcmeDbContext context)
