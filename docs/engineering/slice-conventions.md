...
**Enforced by:** `tests/Architecture/Acme.Architecture.Tests`
...
3. Run `dotnet test tests/Architecture/Acme.Architecture.Tests`.
...
```
src/<Context>/Acme.<Context>.Domain/Aggregates/<Aggregate>/I<Aggregate>Repository.cs   ✅
src/<Context>/Acme.<Context>.Application/Persistence/I<Aggregate>Repository.cs         ❌
```
...
Writes go through the repository. Reads use `AcmeDbContext` directly with
`AsNoTracking()` — a query handler never loads an aggregate.

**Reference:** [Documents/IDocumentRepository.cs](../../src/DocumentManagement/Acme.DocumentManagement.Domain/Aggregates/Documents/IDocumentRepository.cs) ·
...
**Reference:** [DocumentManagement/Commands/UploadDocument/](../../src/DocumentManagement/Acme.DocumentManagement.Application/Commands/UploadDocument/) ·
...
[src/Persistence/Acme.Persistence/](../../src/Persistence/Acme.Persistence/),
not in the context — this is deliberate and predates the contexts.

Endpoints live in [src/Host/Acme.Api/Endpoints/\<Plural\>/](../../src/Host/Acme.Api/Endpoints/),
one file per endpoint, and are registered in
[Program.cs](../../src/Host/Acme.Api/Program.cs) under a `MapGroup` per context.
...
**Reference:** [documents/api/](../../web/acme-web/src/features/documents/api/)
...
**Reference:** [documents/hooks/](../../web/acme-web/src/features/documents/hooks/)

## SC-103 — Every form has a zod schema in `validation/`

```
validation/uploadDocumentSchema.ts    ✅
```
...
**Reference:** [documents/validation/uploadDocumentSchema.ts](../../web/acme-web/src/features/documents/validation/uploadDocumentSchema.ts)

## SC-104 — Dialog and Form are separate components

```
components/UploadDocumentDialog.tsx    ← shell: open state, title, mounting
components/UploadDocumentForm.tsx      ← react-hook-form + zodResolver + fields
```

A dialog holding raw `useState` per field is the signal SC-103 was skipped.

**Reference:** [documents/components/UploadDocumentForm.tsx](../../web/acme-web/src/features/documents/components/UploadDocumentForm.tsx)
...
```
web/acme-web/src/features/<area>/<feature>/
...
> aliases to delete, and which `AcmeDbContext` still carries an alias for.
