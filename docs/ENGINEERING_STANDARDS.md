...
These standards apply across the entire Acme platform.
...
> enforced by `tests/Architecture/Acme.Architecture.Tests`. This document
...
All developers and CI environments should build Acme using the same SDK version to ensure reproducible builds.
...
> ([SharedKernel/Primitives/StronglyTypedId.cs](../src/Shared/Acme.SharedKernel/Primitives/StronglyTypedId.cs)).
...
[`Entity<TId>`](../src/Shared/Acme.SharedKernel/Abstractions/Entity.cs)
...
Reference: [Commitment.cs](../src/Interaction/Acme.Interaction.Domain/Commitments/Commitment.cs)
and [CommitmentId.cs](../src/Interaction/Acme.Interaction.Domain/Commitments/CommitmentId.cs).
...
[HaQuestion.cs](../src/Interaction/Acme.Interaction.Domain/Correspondence/HaQuestion.cs)
and [CorrespondenceAttachment.cs](../src/Interaction/Acme.Interaction.Domain/Correspondence/CorrespondenceAttachment.cs).
The id form is enforced; this base-class rule is still review-time.

> **Enforced by** `IdentityConventionTests` in
> [tests/Architecture/Acme.Architecture.Tests](../tests/Architecture/Acme.Architecture.Tests/IdentityConventionTests.cs).
...
> and `Down`**, and regenerates `AcmeDbContextModelSnapshot.cs` unchanged.
...
git status --short src/Persistence/Acme.Persistence/Migrations/   # must be clean
```
...
`Acme.Persistence` alone does not refresh the copy of it sitting in
`src/Host/Acme.Api/bin`, so `--no-build` runs the differ against the *previous*
