...
> then grew six repository interfaces. This ADR records what Acme actually
...
handlers may use `AcmeDbContext` directly, repositories add abstraction without
value.
...
**Reads use `AcmeDbContext` directly.** Query handlers inject the context and
...
- `AcmeDbContext` is a single shared context (see `docs/ARCHITECTURE_BACKLOG.md`
