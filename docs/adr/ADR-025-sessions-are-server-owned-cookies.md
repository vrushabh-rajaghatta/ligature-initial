...
consequence: Acme cannot show a user their own token, only recognise it.
...
- **Cookies bring CSRF risk that bearer headers do not**, because the browser
  ...
  is the thing to revisit first if Acme ever gains one that is.
- `SameSite=Strict` means a link into Acme from an external site lands signed
...
- Acme gains a flow that must be reachable from another site, or a
