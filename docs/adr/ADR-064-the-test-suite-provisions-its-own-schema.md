...
**27 test files across 7 of Acme's 19 test suites hard-code
`Host=localhost;…;Database=acme`** — the developer's own working database.
...
is compliant; the one below is what Acme builds.
...
assembly provisions one database, named `acme_test_<assembly>_<short-guid>`.
...
- **One line of ADR-019's Consequences is overtaken.** It records that the suite
  *"can run repeatedly against a shared development database"* — true when
  written, and no longer how Acme works. **ADR-019 is not edited**; an accepted
...
- One new project, `tests/TestSupport/Acme.TestSupport`, owns the lifecycle.
