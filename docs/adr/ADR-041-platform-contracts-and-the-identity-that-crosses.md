...
be chased *by Acme*; the RA lead who manages them owns it, and is a user. A
...
### 2. A new project, `Acme.Platform.Contracts`, holds `UserId` and nothing else
...
Moving `UserId` into `Acme.SharedKernel` looks like the `TenantId` precedent and
is not.
...
- `UserId` moves from `Acme.Platform.Domain.Aggregates.User` to
  `Acme.Platform.Contracts`. **93 files mention it, 74 inside Platform**; every
...
- **An owner needs to be someone without a Acme login.** Decision 1's workflow
