...
2. **How is each new entity scoped to a tenant?** Acme had accumulated three
...
answers rather than a guess. The authoritative list lives in `AcmeDbContext`'s
...
3. **Global world facts** — no filter. Acme is describing an external reality
